...
void GUI_App::update_userInfos(bool writen) {
    // update
    ...
    if (m_login_userinfo.get_need_update()) {
        ...
        m_login_userinfo.on_updated();
    }

    if (writen) {
        auto snmk_world = SnapmakerWorld::GetInstance();
        if (snmk_world == nullptr) {
            return;
        }
        for (auto& pair : m_userInfos) {
            if (pair.second.get_need_update()) {
                snmk_world->add_target(pair.second);
                pair.second.on_updated();
            }
        }
    }
}
